...
public class Tra2Emf {
	...
	public static void main(String[] args) {
		...
		try {
			...
			kind = args[2];
			//uniform = Boolean.parseBoolean(args[3]); useless right now
		} catch (Exception e1) {
		...
		for (int i = 0; i < filesInFolder.length; i++) {
			if(!filesInFolder[i].getName().endsWith(".tra")) {
				continue;
			}
	...
	/**
	 * Turn a transition graph defined in a .tra file to a corresponding emf instance.
	 * 
	 * @param traFile
	 * @return a transition graph
	 */
	public static TransitionGraph tra2emf(File traFile) {
	    Tra2EmfConverter converter = new Tra2EmfConverter();
	    TransitionGraph tg = converter.convert(traFile, "PA", true);
	    return tg;
	}
